                                      ...
                                      GError **error)
{
	...
	if (g_path_is_absolute (plugin_filename)) {
		str_release_1 = _nm_utils_check_module_file (plugin_filename,
		                                            check_owner,
		                                            check_file,
		                                            user_data,
		                                            &local);
	...
	if (   !module
		 ...
		 * to the SO in LIBDIR */
		basename = g_path_get_basename (plugin_filename);
		str_release_2 = g_strdup_printf ("%s/NetworkManager/%s", LIBDIR, basename);
		g_free (basename);

		str_release_3 = _nm_utils_check_module_file (str_release_2,
		                                             check_owner,
		                                             check_file,
		                                             user_data,
		                                             NULL);
		...
		if (editor_plugin) {
			...
			if (factory_error) {
				...
				g_set_error (error,
				             NM_VPN_PLUGIN_ERROR,
				             NM_VPN_PLUGIN_ERROR_FAILED,
				             _("unknown error initiating plugin %s"), plugin_filename);
			}
		}
