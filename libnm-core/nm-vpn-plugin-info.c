 ...
 */
GSList *
nm_vpn_plugin_info_load_dir_full (const char *dirname,
                                  gboolean do_validate_filename,
                                  int check_owner,
                                  NMUtilsCheckFilePredicate check_file,
                                  gpointer user_data)
 ...
 * directories. This is a simplified version of nm_vpn_plugin_info_load_dir_full()
 * that iterates first over nm_vpn_plugin_info_get_default_dir_user() and then
 * nm_vpn_plugin_info_get_default_dir_lib(), and lastly nm_vpn_plugin_info_get_default_dir_etc().
 *
 * Since: 1.2
 */
GSList *
nm_vpn_plugin_info_load_dir ()
{
	...
	GSList *infos, *info;
	const char *dir[] = {
		[0] = nm_vpn_plugin_info_get_default_dir_user (),
		[2] = nm_vpn_plugin_info_get_default_dir_lib (),
		[1] = nm_vpn_plugin_info_get_default_dir_etc (),
	};

	uid = getuid ();

	/* by default, we load plugins from NM_VPN_PLUGIN_DIR *and* DEFAULT_DIR*, with
	...
	for (i = 0; i < G_N_ELEMENTS (dir); i++) {
		...
		infos = nm_vpn_plugin_info_load_dir_full (dir[i], TRUE, uid, NULL, NULL);
 ...
 * file permissions with nm_vpn_plugin_info_check_file().
 *
