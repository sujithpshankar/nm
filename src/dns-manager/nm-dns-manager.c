...
add_dns_option_item (GPtrArray *array, const char *str)
{
	...
	if (!_nm_utils_dns_option_parse (str, &name, NULL))
		return;

	if (_nm_utils_dns_option_check_duplicate (array, name))
		g_ptr_array_add (array, g_strdup (str));
...
merge_one_ip4_config (NMResolvConfData *rc, NMIP4Config *src)
{
	...
	for (i = 0; i < num; i++) {
		...
		option = nm_ip4_config_get_option (src, i);
...
merge_one_ip6_config (NMResolvConfData *rc, NMIP6Config *src)
{
	...
	for (i = 0; i < num; i++) {
		...
		option = nm_ip6_config_get_option (src, i);
		add_dns_option_item (rc->options, option);
	}
}
