...
try_add_service (NMVpnManager *self, NMVpnPluginInfo *plugin_info)
{
	...
	if (nm_vpn_plugin_info_list_find_by_filename (priv->services,
		...
		return;
	if (nm_vpn_plugin_info_list_find_by_name (priv->services,
                 ...
                 gpointer user_data)
{
	...
	case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT:
		...
		if (!nm_vpn_plugin_info_check_file (path, TRUE, TRUE, 0,
		                                    NULL, NULL, &error)) {
...
nm_vpn_manager_init (NMVpnManager *self)
{
	...
	GFile *file;
	GSList *infos, *info;
	const char *conf_dir_etc = nm_vpn_plugin_info_get_default_dir_etc ();
	const char *conf_dir_lib = nm_vpn_plugin_info_get_default_dir_lib ();
	...
	infos = nm_vpn_plugin_info_load_dir_full (conf_dir_lib, TRUE, 0, NULL, NULL);
	for (info = infos; info; info = info->next)
		try_add_service (self, info->data);
	g_slist_free_full (infos, g_object_unref);

	infos = nm_vpn_plugin_info_load_dir_full (conf_dir_etc, TRUE, 0, NULL, NULL);
	for (info = infos; info; info = info->next)
		try_add_service (self, info->data);
	g_slist_free_full (infos, g_object_unref);
}
