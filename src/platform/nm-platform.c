...
enum {
	PROP_0,
	PROP_DO_SETUP_SINGLETON,
	LAST_PROP,
};

typedef struct {
	gboolean do_setup_singleton;
} NMPlatformPrivate;
...
nm_platform_link_to_string (const NMPlatformLink *link)
{
	...
	g_snprintf (_nm_platform_to_string_buffer, sizeof (_nm_platform_to_string_buffer),
	            ...
	            link->kind ? (strcmp (nm_link_type_to_string (link->type), link->kind) ? "/" : "*") : "",
	            link->kind && strcmp (nm_link_type_to_string (link->type), link->kind) ? link->kind : "",
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	switch (prop_id) {
	case PROP_DO_SETUP_SINGLETON:
		/* construct-only */
		priv->do_setup_singleton = g_value_get_boolean (value);
		break;
	default:
...
nm_platform_class_init (NMPlatformClass *platform_class)
{
	...
	g_object_class_install_property
	 (object_class, PROP_DO_SETUP_SINGLETON,
	     g_param_spec_boolean (NM_PLATFORM_DO_SETUP_SINGLETON, "", "",
	                           FALSE,
	                           G_PARAM_WRITABLE |
