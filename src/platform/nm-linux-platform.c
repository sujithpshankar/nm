...
typedef struct {
	...
	gint delayed_action_is_handling;
	guint delayed_action_idle_id;
...
gboolean
_nmp_vt_cmd_plobj_init_from_nl_link (NMPlatform *platform, NMPlatformObject *_obj, const struct nl_object *_nlo, gboolean id_only)
{
...
gboolean
_nmp_vt_cmd_plobj_init_from_nl_ip4_address (NMPlatform *platform, NMPlatformObject *_obj, const struct nl_object *_nlo, gboolean id_only)
{
...
gboolean
_nmp_vt_cmd_plobj_init_from_nl_ip6_address (NMPlatform *platform, NMPlatformObject *_obj, const struct nl_object *_nlo, gboolean id_only)
{
...
gboolean
_nmp_vt_cmd_plobj_init_from_nl_ip4_route (NMPlatform *platform, NMPlatformObject *_obj, const struct nl_object *_nlo, gboolean id_only)
{
	...
	obj->scope_nm = nmp_utils_ip_route_scope_native_to_nm (rtnl_route_get_scope (nlo));
...
gboolean
_nmp_vt_cmd_plobj_init_from_nl_ip6_route (NMPlatform *platform, NMPlatformObject *_obj, const struct nl_object *_nlo, gboolean id_only)
{
...
delayed_action_handle_all (NMPlatform *platform)
{
	...
	nm_clear_g_source (&priv->delayed_action_idle_id);
	priv->delayed_action_is_handling++;
...
{
	NM_LINUX_PLATFORM_GET_PRIVATE (user_data)->delayed_action_idle_id = 0;
...
delayed_action_schedule (NMPlatform *platform, DelayedActionType action_type, gpointer user_data)
{
	...
	if (priv->delayed_action_is_handling == 0 && priv->delayed_action_idle_id == 0)
		priv->delayed_action_idle_id = g_idle_add (delayed_action_handle_idle, platform);
...
do_refresh_object (NMPlatform *platform, const NMPObject *obj_needle, NMPlatformReason reason, gboolean handle_all_delayed_actions, const NMPObject **out_obj)
{
	...
	if (klass->obj_type == OBJECT_TYPE_LINK) {
		...
		if (ifindex >= 0)
			delayed_action_prune_all (priv->delayed_actions, DELAYED_ACTION_TYPE_REFRESH_LINK, GINT_TO_POINTER (ifindex), TRUE);
	}
...
do_add_link (NMPlatform *platform, const char *name, const struct rtnl_link *nlo)
{
	...
	do_refresh_object (platform, &obj_needle, NM_PLATFORM_REASON_INTERNAL, TRUE, NULL);
...
link_add (NMPlatform *platform, const char *name, NMLinkType type, const void *address, size_t address_len)
{
	...
	return do_add_link (platform, name, (struct rtnl_link *) l);
}
...
link_get_ifindex (NMPlatform *platform, const char *ifname)
{
	...
	list = nmp_cache_lookup_multi (priv->cache,
	                               nmp_cache_id_init_object_type (NMP_CACHE_ID_STATIC, OBJECT_TYPE_LINK),
	                               &len);
	for (i = 0; i < len; i++) {
		const NMPObject *obj = NMP_OBJECT_UP_CAST (list[i]);

		if (!nmp_object_is_visible (obj))
			continue;
		if (!strcmp (ifname, obj->link.name))
			return obj->link.ifindex;
	}
	return 0;
}
...
vlan_add (NMPlatform *platform, const char *name, int parent, int vlan_id, guint32 vlan_flags)
{
	...
	return do_add_link (platform, name, rtnllink);
}
...
infiniband_partition_add (NMPlatform *platform, int parent, int p_key)
{
	...
	do_refresh_object (platform, &obj_needle, NM_PLATFORM_REASON_INTERNAL, TRUE, &obj_cache);
...
ip4_route_delete (NMPlatform *platform, int ifindex, in_addr_t network, int plen, guint32 metric)
{
	...
	if (!_nl_has_capability (1 /* NL_CAPABILITY_ROUTE_BUILD_MSG_SET_SCOPE */)) {
		...
		if (obj)
			scope = nmp_utils_ip_route_scope_nm_to_native (obj->ip4_route.scope_nm);
                     ...
                     GUdevDevice *udev_device)
{
	...
	if (g_udev_device_get_property (udev_device, "IFINDEX"))
		 ...
		 */
		links = (const NMPObjectLink *const *) nmp_cache_lookup_multi (priv->cache, nmp_cache_id_init_links (NMP_CACHE_ID_STATIC, FALSE), NULL);
		if (links) {
			for (; *links; links++) {
