...
_link_get_driver (GUdevDevice *udev_device, const char *kind, const char *ifname)
{
	...
	if (ifname) {
		driver = nmp_utils_ethtool_get_driver (ifname);
		if (driver)
			return driver;
	}
...
void
_nmp_object_fixup_link_udev_fields (NMPObject *obj)
{
	const char *driver = NULL;
	const char *udi = NULL;
	gboolean is_in_udev = FALSE;
	...
	obj->link.is_in_udev = is_in_udev;
}
...
NMPObject *
nmp_object_ref (NMPObject *object)
{
	g_return_val_if_fail (NMP_OBJECT_IS_VALID (object), NULL);
	g_return_val_if_fail (object->_ref_count != NMP_REF_COUNT_STACKINIT, NULL);
	object->_ref_count++;
...
void
nmp_object_unref (NMPObject *object)
{
	if (object) {
		g_return_if_fail (object->_ref_count > 0);
		g_return_if_fail (object->_ref_count != NMP_REF_COUNT_STACKINIT);
		if (--object->_ref_count <= 0) {
			const NMPClass *klass = object->_class;
 ...
 **/
NMPObject *
nmp_object_from_nl (NMPlatform *platform, const struct nl_object *nlo, gboolean id_only)
{
	...
	if (!NMP_OBJECT_GET_CLASS (obj)->cmd_plobj_init_from_nl (platform, &obj->object, nlo, id_only)) {
...
_vt_cmd_obj_init_cache_id_link (const NMPObject *obj, NMPCacheIdType id_type, NMPCacheId *id, const NMPCacheId **out_id)
{
	...
	case NMP_CACHE_ID_TYPE_OBJECT_TYPE:
		*out_id = nmp_cache_id_init_object_type (id, OBJECT_TYPE_LINK);
		return TRUE;
...
_vt_cmd_obj_init_cache_id_ip4_address (const NMPObject *obj, NMPCacheIdType id_type, NMPCacheId *id, const NMPCacheId **out_id)
{
	switch (id_type) {
	case NMP_CACHE_ID_TYPE_OBJECT_TYPE:
		*out_id = nmp_cache_id_init_object_type (id, OBJECT_TYPE_IP4_ADDRESS);
		return TRUE;
...
_vt_cmd_obj_init_cache_id_ip6_address (const NMPObject *obj, NMPCacheIdType id_type, NMPCacheId *id, const NMPCacheId **out_id)
{
	switch (id_type) {
	case NMP_CACHE_ID_TYPE_OBJECT_TYPE:
		*out_id = nmp_cache_id_init_object_type (id, OBJECT_TYPE_IP6_ADDRESS);
		return TRUE;
	case NMP_CACHE_ID_TYPE_ADDRROUTE_BY_IFINDEX:
		if (_vt_cmd_obj_is_visible_ipx_address (obj)) {
...
_vt_cmd_obj_init_cache_id_ip4_route (const NMPObject *obj, NMPCacheIdType id_type, NMPCacheId *id, const NMPCacheId **out_id)
{
	switch (id_type) {
	case NMP_CACHE_ID_TYPE_OBJECT_TYPE:
		*out_id = nmp_cache_id_init_object_type (id, OBJECT_TYPE_IP4_ROUTE);
		return TRUE;
	case NMP_CACHE_ID_TYPE_ADDRROUTE_BY_IFINDEX:
...
_vt_cmd_obj_init_cache_id_ip6_route (const NMPObject *obj, NMPCacheIdType id_type, NMPCacheId *id, const NMPCacheId **out_id)
{
	switch (id_type) {
	case NMP_CACHE_ID_TYPE_OBJECT_TYPE:
		*out_id = nmp_cache_id_init_object_type (id, OBJECT_TYPE_IP6_ROUTE);
		return TRUE;
	case NMP_CACHE_ID_TYPE_ADDRROUTE_BY_IFINDEX:
...
NMPCacheOpsType
nmp_cache_remove (NMPCache *cache, const NMPObject *obj, NMPObject **out_obj, gboolean *out_was_visible, NMPCachePreHook pre_hook, gpointer user_data)
{
	NMPObject *old;
...
nmp_cache_remove_netlink (NMPCache *cache, const NMPObject *obj_needle, NMPObject **out_obj, gboolean *out_was_visible, NMPCachePreHook pre_hook, gpointer user_data)
{
	if (NMP_OBJECT_GET_TYPE (obj_needle) == OBJECT_TYPE_LINK) {
		...
		_nmp_object_fixup_link_udev_fields (obj);
		...
		return nmp_cache_remove (cache, obj_needle, out_obj, out_was_visible, pre_hook, user_data);
}
...
ASSERT_nmp_cache_is_consistent (const NMPCache *cache)
{
	...
	while (g_hash_table_iter_next (&iter_hash, (gpointer *) &obj, NULL)) {
		...
		for (id_type = 0; id_type <= NMP_CACHE_ID_TYPE_MAX; id_type++) {
			if (!NMP_OBJECT_GET_CLASS (obj)->cmd_obj_init_cache_id (obj, id_type, &cache_id_storage, &cache_id))
				continue;
			if (!cache_id)
				continue;
	...
	while (nm_multi_index_iter_next (&iter_multi,
		...
		for (i = 0; i < len; i++) {
			...
			if (!NMP_OBJECT_GET_CLASS (obj)->cmd_obj_init_cache_id (obj, cache_id->_id_type, &cache_id_storage, &cache_id2))
				g_assert_not_reached ();
			g_assert (cache_id2);
