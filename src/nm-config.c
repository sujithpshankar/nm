                     ...
                     GError **error)
{
	...
	g_key_file_set_comment (keyfile, NULL, NULL,
	                        ...
	                        " The value from this file is ignored if the corresponding \""NM_CONFIG_KEYFILE_KEYPREFIX_WAS"\" key is no longer\n"
	                        " equal with the current configuration as read from NetworkManager.conf.\n"
	                        " That means, if you modify a value in NetworkManager.conf, the internal\n"
	                        " configuration value no longer matches and is ignored.\n"
	                        "\n"
	...
	data = g_key_file_to_data (keyfile, &data_len, &local);
	if (!data)
		goto out;

	if (!g_file_set_contents (filename, data, data_len, &local))
		goto out;
...
void
nm_config_set_values_intern (NMConfig *self,
                             gboolean allow_write,
                             gboolean force_rewrite,
                             ...)
{
	...
	/* variadic argument accept a tripple of "group","key","value.
	 * Group and key cannot be missing or empty. A value of %NULL
	 * means to remove the key. */

	va_start (ap, force_rewrite);
	group = va_arg (ap, char *);
	if (group) {
		keyfile = nm_config_data_clone_keyfile_intern (priv->config_data);
		do {
			gs_free char *group_intern;
			...
			group_intern = g_strconcat (NM_CONFIG_KEYFILE_GROUPPREFIX_INTERN, group, NULL);
			if (value)
				g_key_file_set_string (keyfile, group_intern, key, value);
			else
				g_key_file_remove_key (keyfile, group_intern, key, NULL);
		} while ((group = va_arg (ap, char *)));
	}
	va_end (ap);

	if (!keyfile) {
		if (allow_write && force_rewrite)
			nm_config_set_values (self, _nm_config_data_get_keyfile_intern (priv->config_data),
			                      allow_write, force_rewrite);
		return;
	}

	nm_config_set_values (self,
	                      keyfile,
	                      allow_write,
	                      force_rewrite);
	g_key_file_unref (keyfile);
